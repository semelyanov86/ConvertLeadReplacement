...
Vtiger_Detail_Js("Leads_Detail_Js",{
	...
	convertLeadToSalesOrder : function(convertLeadUrl, buttonElement) {
		var instance = Leads_Detail_Js.detailCurrentInstance;
		//Initially clear the elements to overwtite earliear cache
		instance.convertLeadContainer = false;
		instance.convertLeadForm = false;
    ...
    registerEvents : function(){
        ...
        //PINstudio #1064
        var $soStatus = $('[name=leadstatus]');
        if ($soStatus.length == 0) return;

        $soStatus[0].onchange = function(){
            var value = this.value;
            if (value != 'Отказался') return;

            Vtiger_Detail_Js.forceReason('cf_994');
        }
        //PINstudio end #1064
    }
