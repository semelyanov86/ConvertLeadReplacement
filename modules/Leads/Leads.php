...
class Leads extends CRMEntity {
	...
	*/
    ...
    // PINstudio begin Copy from Accounts to handle Calls list @oleg #25716-17
	/**
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		if ($numOfFields > 0) {
			...
			//PINstudio added
			$dependentTable = $this->db->query_result($dependentFieldSql, 0, 'tablename');
			...
			$entityIds = $id;//$this->getRelatedContactsIds();
			...
			$query .= " WHERE vtiger_crmentity.deleted = 0 AND $dependentTable.$dependentColumn IN (".$entityIds.")"; // PINstudio Added, to use right table to look for dependent field @oleg #25716-18
