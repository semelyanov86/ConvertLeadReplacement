...
class Leads_DetailView_Model extends Accounts_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		$paymentsModuleModel = Vtiger_Module_Model::getInstance('SPPayments');
		if(!empty($paymentsModuleModel)) {
			$basicActionLink = array(
				'linktype'  => 'DETAILVIEWBASIC',
				'linklabel' => '+ Платёж',
				'linkurl'   => 'javascript:Vtiger_Detail_Js.triggerQuickPayment();',
				'linkicon'  => '',
				'sequence'  => 3
			);
		...
		if(Users_Privileges_Model::isPermitted($moduleModel->getName(), 'ConvertLead', $recordModel->getId()) && Users_Privileges_Model::isPermitted($moduleModel->getName(), 'EditView', $recordModel->getId())) {
			...
			$basicActionLink = array(
				'linktype' => 'DETAILVIEWBASIC',
				'linklabel' => 'Оформить заказ',
				'linkurl' => 'Javascript:Leads_Detail_Js.convertLeadToSalesOrder("'.$recordModel->getconvertLeadToSalesOrderUrl().'",this);',
				'linkicon' => ''
			);
			$linkModelList['DETAILVIEWBASIC'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
			// PINstudio end
