...
class Leads_Record_Model extends Vtiger_Record_Model {
	...
	function getconvertLeadToSalesOrderUrl() {
		return 'index.php?module='.$this->getModuleName().'&view=ConvertLeadToSalesOrder&record='.$this->getId();
	}
	// PINstudio end
		...
		if ($moduleModel->isActive()) {
			...
			foreach ($fieldModels as $fieldName => $fieldModel) {
				if($fieldModel->isMandatory() && $fieldName != 'assigned_user_id') {
					...
					// PINstudio begin Fix setting default values @binizik #25716-16
